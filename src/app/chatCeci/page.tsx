...
export default function ChatCeci() {
  ...
  const listarRelatorios = async () => {
    ...
    try {
      const response = await fetch('http://localhost:5000/reports/list', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const contemPDF = (texto: string) => {
    return texto.includes('📁 Arquivo:') || texto.includes('PDF gerado') || texto.includes('.pdf');
  };
                    ...
                    <div className="flex justify-between items-center">
                      <div>
                        ...
                        <p className="text-xs text-gray-500">
                          {new Date(report.created * 1000).toLocaleString()} | {Math.round(report.size/1024)}KB
                        </p>
                      </div>
                      <button
        ...
        {conversa.map((msg, i) => (
          <div key={i} className={`flex ${msg.autor === "usuário" ? "justify-end" : "justify-start"} items-start`}>
            ...
            <div className="max-w-[70%]">
              ...
              {msg.autor === "ceci" && (contemPDF(msg.texto) || msg.pdfInfo) && (
                <div className="mt-2">
                  <button
                    onClick={() => {
                      const filename = msg.pdfInfo?.filename || extrairNomePDF(msg.texto);
